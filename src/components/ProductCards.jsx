  ...
  return (
    <div className="grid grid-cols-1 gap-4 px-[5%] py-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
      {products.map((product) => (
        <div
          key={product.id}
          className="group overflow-hidden rounded-2xl border border-slate-200 shadow-sm transition-all hover:border-green-700 hover:shadow-md"
        >
...
function ActionCards({ product }) {
  return (
    <div className="mt-auto flex items-center gap-2 pt-4">
      ...
      <button className="flex-1 cursor-pointer rounded-lg border border-slate-300 px-3 py-2 text-center text-xs font-semibold text-slate-700 transition-colors hover:border-green-700 hover:bg-green-700 hover:text-white">
        Buy Now
      </button>
    </div>
  );
