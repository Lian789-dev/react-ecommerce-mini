...
export default function PaymentMethod() {
  ...
  const onSelectedPayment = useCheckoutStore(
    (state) => state.onSelectedPayment
  );
  const payment = [
    {
      id: "qris",
      ...
      <div className="flex flex-col gap-2">
        {payment.map((item) => (
          <button
            ...
            className={`flex items-center gap-2 p-2 text-sm ${selectedPayment === item.id ? "bg-slate-200" : "bg-transparent"}`}
          >
            {item.icon}
            <p>{item.title}</p>
          </button>
